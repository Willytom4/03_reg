...
# standardized residual help us to identify outliers. To bring back the remaining data on the table, we use the left_join(data)
```
...
ggplot(mod1_int_aug,
  ...
  geom_hline(yintercept = 0,
             color = "red") + 
  geom_smooth()
#geom_smooth helps to visualize the trend. here we see a pattern of increasing residuals as our input increase. We dont want that.
```
...
ggplot(mod1_int_aug,
  ...
  stat_qq() +
  stat_qq_line()
# qq plot helps to determine if our residuals are normally distributed. Most of the lines must fit around the line
```
...
#lmer allows us to run mixed effect
```
...
# we can use geom_point instead of geom_line because we are connecting the predicted points
...
# nlme for non linear model. The starting point is important
```
...
#outliers is any point greater or less than 3
```
Looking good.  
...
# lack of variance for b and xs gives zero
```
...
IC_tab(mod1_int,
       ...
       mod3_quad,
       mod4_linp)
#dAIC assigns 0 to the best model and report the difference for other models to know how far we are from the best model. Likelihood of the model fit given your data is AIC. It can be a negative value. If the AIC point is not more than 2 points, it is difficult to assign the best model to any of the two models.
```
...
#geom_segment helps to draw the lines on x axis and y axis. i.e. to draw the line for the yield and seedling rate
```
# 9) Take-home  
